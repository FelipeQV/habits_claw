-- Interpretation-first input template for analysis.
-- Block 1 gives day totals vs active target for {{analyze_date}}.
WITH t AS (
    SELECT
        nutrition_target_id,
        kcal_target,
...
d AS (
    ...
    FROM meals
    WHERE corrected = 0
      AND date = {{analyze_date}}
)
SELECT
    {{analyze_date}} AS analyze_date,
    d.calories_actual,
    d.protein_actual,
    d.carbs_actual,
    d.fat_actual,
...
-- Block 2 (optional): breakdown by meal_type for interpretation.
SELECT
    meal_type,
    COALESCE(SUM(calories), 0) AS calories_actual,
    COALESCE(SUM(protein), 0) AS protein_actual,
